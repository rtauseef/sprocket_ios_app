...
-(void) resolvePayoffFromMetadata:(PGPayoffMetadata *) meta completion:(void(^__nonnull)(BOOL success)) completion {
    if(meta.type == kPGPayoffURL && meta.URL) {
                ...
                [api requestImageMetadataWithUUID:[meta.data objectForKey:kPGPayoffUUIDKey] completion:^(NSError * _Nullable error, PGMetarMedia * _Nullable metarMedia) {
                    ...
                    if (metarMedia.artifacts) {
                        for (PGMetarArtifact *artifact in metarMedia.artifacts) {
                            if (artifact.type == PGMetarArtifactTypeAura) {
                                arArtifact = artifact;
                            }
                        }
                        ...
                        if (arArtifact != nil) {
                            ...
                            // START AURA HANDLING for Aura ID
                             [self cacheAndStartDetachedAura:arArtifact.auraID];
                        } else {
                            dispatch_async(dispatch_get_main_queue(), ^{
                                ...
                                [self presentViewController:metarViewController animated:YES completion:^{
                                    completion(YES);
                                }];
                            });
                        }
