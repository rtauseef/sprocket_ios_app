...
class FlickrPhotoScreen < Calabash::IBase
 ...
 def photo_width
  ...
  def navigate
    ...
    if not current_page?
        flicr_albm_screen = go_to(FlickrAlbumScreen)
        wait_for_elements_exist(flicr_albm_screen.flickr_first_album, :timeout => MAX_TIMEOUT)
        touch flicr_albm_screen.flickr_first_album
      end
    await
  end
